...
#| fig-alt: "Html table with less rows"
smartpill %>%  
  tbl_summary(
...
#| fig-alt: "Html table with different calculated variables"
smartpill %>%  
  tbl_summary(
    include = c(Group, Age, SB.Time, WG.Time), 
    statistic = list(
...
#| fig-alt: "Html table with updated row and NA labels"
smartpill %>%  
  tbl_summary(
    include = c(Group, Age, SB.Time, WG.Time), 
    statistic = list(
...
```{r}
#| fig-alt: "Html table with updated row and NA labels, repeated"
smartpill %>%  
  tbl_summary(
    include = c(Group, Age, SB.Time, WG.Time), 
    statistic = list(
...
```{r label='crosstab'}
#| fig-alt: "Html table with columns for groups"
smartpill %>%  
  tbl_summary(
    by = Group,
...
```{r}
#| fig-alt: "Table with column names listed"
smartpill %>%  
  tbl_summary(
    by = Group,
...
```{r label='col_names'}
#| fig-alt: "Html table with columns for groups, with updated column names"
smartpill %>%  
  mutate(Group = as.factor(Group)) %>%
  tbl_summary(
    by = Group,
...
#| fig-alt: "Html table with new column for p-value"
Table1 %>% 
  #add p-value
  add_p()
```
...
#| fig-alt: "Html table with different p-values"
Table1 %>% 
...
For some customization, we'll need to convert our `gtsummary` object to a `gt` object with the [`as_gt()` function](https://www.danieldsjoberg.com/gtsummary/reference/as_gt.html). 

![https://gt.rstudio.com/reference/figures/gt_parts_of_a_table.svg](https://gt.rstudio.com/reference/figures/gt_parts_of_a_table.svg){fig.alt="Representation of gt table components"}

For instance, to update the footnote to clarify which test was used on which variable.

```{r label='stat_compare_3'}
#| fig-alt: "Html table with footnote"
Table1 %>% 
  add_p(test = Age ~ "t.test") %>% 
...
```{r label='styling'}
#| fig-alt: "Html table with alternating gray rows"
Table1 %>% 
  add_p(test = Age ~ "t.test") %>% 
  as_gt(include = -tab_footnote) %>% 
  tab_footnote(footnote = "t.test for Age; Wilcox rank sum for others",
